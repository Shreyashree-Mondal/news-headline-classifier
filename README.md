...
Classifies news headlines as **Politics**, **Business** or **Sports** using neural networks on pretrained Word2Vec embeddings. The best model (a 2-layer LSTM) is served through a containerized **FastAPI** service, with a **Streamlit** web app in front of it.

- **Live demo:** `<Streamlit app URL>`
- **API docs:** `<Hugging Face Space URL>/docs`
...
```
Headline ──► Streamlit app ──HTTP──► FastAPI (Docker) ──► clean → tokenize → pad ──► LSTM ──► category + probabilities
```
...
streamlit run app/streamlit_app.py     # uses http://localhost:8000 by default
...
Python, TensorFlow/Keras, gensim (Word2Vec), scikit-learn, FastAPI, Pydantic, Docker, Streamlit, pytest, GitHub Actions, Hugging Face Spaces.
